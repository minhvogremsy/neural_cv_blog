// Biaspaltsau A. 22/04/2022 nanodet tensorrt v.01
...
NanoDet_Plus::NanoDet_Plus(string model_path, string classesFile, int imgsize, float nms_threshold, float objThreshold)
{
	...
	this->detector = new ObjectDetector_fp16(416, 416);
	this->detector->LoadEngine("../nanodet-plus-m_320.trt");
}
...
void NanoDet_Plus::detect(Mat& srcimg)
{
	...
	const float* preds = (float*)outs_trrt.data;
...
int main()
{
	NanoDet_Plus mynet("../onnxmodel/nanodet-plus-m_320.onnx", "../onnxmodel/coco.names", 320, 0.6, 0.4);       /// choice = ["onnxmodel/nanodet-plus-m_320.onnx", "onnxmodel/nanodet-plus-m_416.onnx",
